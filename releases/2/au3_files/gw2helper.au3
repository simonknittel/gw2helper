	...
	If $local_version < $remote_version Then
		...
		If $do_update == 6 Then ; update.exe herunterladen
			...
			If $download <> 0 And $latest_download <> "" Then
				...
				Exit
			Else
				MsgBox(Default, "GW2 Helper", "Beim Herunterladen der neuen Version ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut, oder laden Sie diese manuell unter " & $latest_download & ".zip herunter.")
			EndIf
		EndIf
	EndIf
EndIf
...
Global $fMain = GUICreate("GW2 Helper", 420, 90, $gui_left, $gui_top, $WS_POPUPWINDOW, $WS_EX_TOPMOST)
...
Global $lDrag = GUICtrlCreateLabel("", 0, 0, 420, 90, -1, $GUI_WS_EX_PARENTDRAG)
